...
$sql= "Select * from `patrol` where ID=$ID";
...
if (move_uploaded_file($tmp_name, $Location.$evidence)) {
    $sql = mysqli_query($con,"UPDATE `patrol` SET ID='$ID', sector='$sector', date='$date', edate='$edate', commander='$commander', escort='$escort', type='$type', des1='$des1', location='$location', casualties='$casualties', actions='$actions', entities='$entities', des='$des', evidence='$evidence' WHERE ID = '$ID'");
 if ($sql) {
      
      echo "<script>alert('patrol is succesfully Updated')</script>";
      header('location:Viewpatrol.php');
  } 
  else{
    echo mysqli_error($con);
    echo "<script>alert('patrol failed to Update')</scrip>";
    } 
  }
                     ...
                     <div class="row">
                         <div class="col-md-8" style="margin-left: 20%;box-shadow:grey;">
                            <div class="text  text-success panel-heading">
                                <h1 style="text-align: center;">Edit patrols Here</h1>
                            </div>
                            <div class="panel-body">
                                <form method="POST" class="form-horizontal" enctype='multipart/form-data'>
                                    ...
                                    <div class="form-group">
                                         <div class="col-md-10">
                                             <button name="Update" class="btn btn-md btn-success col-md-12">Update Patrol</button>
                                         </div>
                                    </div>
                                </form>
                            </div>
